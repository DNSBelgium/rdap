...
public class SearchDomainsControllerTest extends AbstractControllerTest {
  ...
  public void testSearchByNameNotFound() throws Exception {
    when(domainService.searchDomainsByName(NAME)).thenReturn(null);
  ...
  public void testSearchByNameSuccess() throws Exception {
    ...
    domains.add(new Domain(someLinks(), someNotices(), someRemarks(), "en", Domain.OBJECT_CLASS_NAME, someEvents(), someStatuses(),
  ...
  public void testSearchByNsLdhName() throws Exception {
    List <Domain> domains = new ArrayList<Domain>();
    domains.add(new Domain(null, null, null, "en", Domain.OBJECT_CLASS_NAME, null, someStatuses(), null, "Handle", DomainName.of("notexample.org"), null,
    ...
    mockMvc.perform(get("/domains?nsLdhName=" + NS_LDH_NAME).accept(MediaType.parseMediaType("application/rdap+json")))
            .andExpect(status().isOk())
            .andExpect(header().string("Content-type", "application/rdap+json;charset=UTF-8"))
            .andExpect(content().string("{\"rdapConformance\":[\"rdap_level_0\"],\"domainSearchResults\":[{\"lang\":\"en\",\"objectClassName\":\"domain\",\"status\":[\"active\",\"delete prohibited\",\"some specific status\"],\"handle\":\"Handle\",\"ldhName\":\"notexample.org\"}]}"));
