...
public class Contact {
  ...
  public static abstract class Parameters {
    ...
    public static class Builder {
      ...
      public Builder add(String key, String value) {
        if (value.contains("\"")) throw new IllegalArgumentException("value must not contain DQUOTE");
      ...
      public Builder addPref(int pref) {
        if (pref < 1 || pref > 100) throw new IllegalArgumentException("pref must be an integer between 1 and 100");
   ...
   * @return a list of properties having the same name
   */
  public List<Property> getProperties(String name) {
    if (name == null) throw new NullPointerException("name should not be null");
    List<Property> plist = new ArrayList<Property>();
    for (Property property : properties) {
      if (property.name.equalsIgnoreCase(name)) plist.add(property);
    }
    return plist;
  }
  ...
  public static class Builder {
    ...
    public Contact build() {
      if (StringUtils.isEmpty(formattedName)) throw new IllegalStateException("formattedName must not be empty");
      List<Property> properties = new ArrayList<Property>();
      if (!StringUtils.isEmpty(this.familyName) || !StringUtils.isEmpty(givenName))
        properties.add(new Property.N(this.familyName, this.givenName, "", "", ""));
    ...
    public Builder addStreet(String street) {
      if (StringUtils.isEmpty(street)) return this;
      if (this.street == null) this.street = new ArrayList<String>();
      this.street.add(street);
      return this;
    }

    public Builder addOU(String organizationalUnit) {
      if (StringUtils.isEmpty(organizationalUnit)) return this;
