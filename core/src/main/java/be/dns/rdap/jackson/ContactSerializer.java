...
public class ContactSerializer extends JsonSerializer<Contact> {
  ...
  public void serialize(Contact contact, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) throws IOException, JsonProcessingException {
    ...
    for (Contact.Property property : contact.getProperties()) {
      ...
      if (property.getParameters() != null) {
        ...
        while (it.hasNext()) {
          String k = it.next();
          if (k.equalsIgnoreCase("value")) continue;
          Set<String> values = (Set<String>) property.getParameters().get(k);
          if (values.size() == 0) {
          ...
          if (values.size() == 1) {
            jsonGenerator.writeString(values.toArray(new String[0])[0]);
            continue;
          }
          // start write all property parameter values (array)
          jsonGenerator.writeStartArray();
          for (String str : (Set<String>) property.getParameters().get(k))
      ...
      if (property.getParameters() != null) {
        ...
        if (types != null) value = types.iterator().next();
      }
  ...
  @Override
  public void serializeWithType(Contact value, JsonGenerator jgen, SerializerProvider provider, TypeSerializer typeSer) throws IOException, JsonProcessingException {
    super.serializeWithType(value, jgen, provider, typeSer);    //To change body of overridden methods use File | Settings | File Templates.
  }
}
